...
(function () {
    ...
    var fromLocalStorage = function (data) {
        ...
        setItem('Block', data.Block);
        setItem('Panchayat', data.Panchayat);
       // setItem('Village', data.Village);
    };
    ...
    if (len > 0) {
        for (var i = 0; i < len; i++) {
            switch (data.rows.item(i).KeyName) {
                ...
                case 'CmsBlock':
                    $('#DivBlock').html(data.rows.item(i).KeyValue);
                    break;

                case 'CmsPanchayat':
                    $('#DivPanchayat').html(data.rows.item(i).KeyValue);
                    break;
