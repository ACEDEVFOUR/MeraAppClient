...
function querySearchBeneficiary(tx) {
    ...
    //queryText = queryText + " AND [Beneficiary] = " + Beneficiary + " AND ben.FirstName = COALESCE(NULLIF('" + FirstName + "', ''), ben.FirstName) AND ";
    //queryText = queryText + " (ben.HusbandsName = COALESCE(NULLIF('" + HusbandName + "', ''), ben.HusbandsName) OR ";
    //queryText = queryText + " ben.FathersName = COALESCE(NULLIF('" + HusbandName + "', ''), ben.fathersname)) AND ";
    //queryText = queryText + " (ben.IDProof = COALESCE(NULLIF(" + IDProof + ", 0), ben.IDProof) ";
    //queryText = queryText + " AND ben.IDDetails = COALESCE(NULLIF('" + IDDetails + "', ''), ben.IDDetails)) ";
...
function querySearchBeneficiarySuccess(tx, data) {
    ...
    if (len > 0) {
        for (var i = 0; i < len; i++) {
            ...
            //html += '<span class="profile-bene-box body5 text-align-c height25">';
            //html += '<a style="cursor: pointer;" onclick=searchbyprofile(' + data[j].Id + ')';
            //html += '<span class="display-in-b">';
            //html += data[j].FirstName + '</span>';
            //html += '</span>';
            //if (i >= 3) {
            //    html += '<div class="clearboth0"></div>';
            //    i = 0;
