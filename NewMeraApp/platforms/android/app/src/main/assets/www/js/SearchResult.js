...
function querySearchResult(tx) {
    ...
    if (searchFrom != undefined && searchFrom != "") {
        if (searchSchemeObj != "" && searchSchemeObj != undefined) {
            ...
            //    var OrQuery = " (',' || s.Sex  || ',') LIKE '%," + AnySexId + ",%' ";
            //    if (whereClause != '') {
            //        if (AnySexId == 1005 || AnySexId == 1006) {
            ...
            //    }
            //}
            var SexAnyId = getSearchMappingId('sex');
            ...
            if (searchFrom != undefined && searchFrom == "3") {
                ...
                if (searchSchemeObj.Keywords != '') {
                    ...
                    //            var KeywordsQuery ="";
                    //            var mainKeywordQuery = ''
                    //            var keywrds = searchSchemeObj.Keywords.split(',');

                    //            for (var i = 0; i < keywrds.length; i++) {

                    //                if (mainKeywordQuery = '')
                    //                {
                    //                    query = " s.Keywords like '%'"+ left(searchSchemeObj.Keywords, charindex(',', @Keywords+',')-1) +'%'' ';
                    //                }
                    //                else 
                    //                {
                    //                    SET @query = ' or  s.Keywords like ''%'+ left(@Keywords, charindex(',', @Keywords+',')-1) +'%''  '

                    //                    print @mainKeywordQuery
                    //                    --print left(@S, charindex(',', @S+',')-1)
                    //                    --exec YourSP left(@S, charindex(',', @S+',')-1)
                    //                    set @Keywords = stuff(@Keywords, 1, charindex(',', @Keywords+','), '')
                    //                    set @mainKeywordQuery = @mainKeywordQuery + @query
                    //                    end
                    //                }
                    ...
                    //            print @mainKeywordQuery
                    //            if (@mainKeywordQuery = '')
                    //                SET @query = ' s.Keywords like ''%'+ left(@Keywords, charindex(',', @Keywords+',')-1) +'%'' '
                    //        else 
                    //SET @query = ' or  s.Keywords like ''%'+ left(@Keywords, charindex(',', @Keywords+',')-1) +'%''  '
       ...
       //queryText = queryText + " where  s.LangId = 1  AND (',' || s.Sex || ',') LIKE '%,Female,%' ";
       //queryText = queryText + " where  s.LangId = 1  AND s.Sex LIKE '%,Female,%' ";

        //var queryRecentSearchText = "insert or replace RecentSchemes (SchemeId, UserId, LastUpdatedDate, LangId) values ";

        //queryRecentSearchText = queryRecentSearchText + " (( SELECT s.ID FROM Scheme s ";
        ...
        //queryRecentSearchText = queryRecentSearchText + " ), '" + user.userName + "', SELECT date('now') ," + searchSchemeObj.LangId + "  ) ";

        tx.executeSql(queryText, [], querySearchResultSuccess, errorCB);
        //tx.executeSql(queryRecentSearchText, [], queryRecentSearchSuccess, errorCB);
    }
}
...
function querySearchResultSuccess(tx, data) {
    ...
    if (len > 0) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (LangId == 1) {
                searchResultData = searchResultData + '<div><div class="anchor3" style="cursor: pointer;" onclick="ShowScheme(' + data.rows.item(i).ID + ')">Read more...</div></div>';
            }
            else if (LangId == 2) {
                $("#divSearchResultText").html('खोज के परिणाम');
                searchResultData = searchResultData + '<div><div class="anchor3" style="cursor: pointer;" onclick="ShowScheme(' + data.rows.item(i).ID + ')">विवरण पढ़िए...</div></div>';
            }
